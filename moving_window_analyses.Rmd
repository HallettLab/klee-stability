...
stab <- left_join(commstab, treats, by = "Unique_ID") %>% #join comm stability with treatment info
  mutate(CV = 1/stability) %>% #calculate the CV as inverse of stability
  group_by(TREATMENT) %>%
  summarize(mean_st = mean(stability), SE_st = calcSE(stability))


s <- ggplot(stab, aes(x=TREATMENT, y=mean_st)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin = mean_st-SE_st, ymax=mean_st+SE_st), width = 0.2) + #add standard error bars
  ylab("Stability (full timeseries)") + xlab("Treatment") + #label axes
...
sdreg5 <- ggplot(figdrst5, aes(x=Dscore, y=stability)) +
  geom_point() +
  ylab("Stability (5yr)") + xlab("Drought Score") +
  geom_point(aes(fill=TREATMENT), 
             colour="black",pch=21, size=2.5) +
  scale_fill_scico_d(palette = "batlow", direction = -1) +
  scale_color_scico_d(palette = "batlow", direction = -1) +
...
```{r, fig.height=3.75, fig.width=8.5}



vrdreg <- ggplot(figdrst, aes(x=Dscore, y=cVR, color = TREATMENT)) +
  geom_point() +
...
psdreg <- ggplot(figdrst, aes(x=Dscore, y=popstab, color = TREATMENT)) +
  geom_point() +
  ylab("Population Stability (10yr)") + xlab("Drought Score") +
  geom_point(aes(fill=TREATMENT), 
             colour="black",pch=21, size=2.5) +
  scale_fill_scico_d(palette = "batlow", direction = -1) +
  scale_color_scico_d(palette = "batlow", direction = -1) +
...
ridreg <- ggplot(figdrst, aes(x=Dscore, y=richness, color = TREATMENT)) +
  geom_point() +
  ylab("Species Richness (10yr)") + xlab("Drought Score") +
  geom_point(aes(fill=TREATMENT), 
             colour="black",pch=21, size=2.5) +
  scale_fill_scico_d(palette = "batlow", direction = -1) +
  scale_color_scico_d(palette = "batlow", direction = -1) +
...
ggplot(figdrst5, aes(x=Dscore, y=richness, color = TREATMENT)) +
  geom_point() +
  ylab("Species Richness (5yr)") + xlab("Drought Score") +
  geom_point(aes(fill=TREATMENT), 
             colour="black",pch=21, size=2.5) +
  scale_fill_scico_d(palette = "batlow", direction = -1) +
  scale_color_scico_d(palette = "batlow", direction = -1) +
...
ggarrange(vrdreg, psdreg, ridreg,
          ncol = 3, nrow = 1, 
          common.legend = TRUE, 
          legend = "bottom", 
          align = "hv", 
