...
################################################################################
#### SC June 2007 Data Cleaning ################################################
################################################################################
SC <- read.csv("2007June_SC_raw.csv", skip = 4) ##manually filled trap station names, can do this in R for other datasheets
...
#flip to long format
SC_long <- pivot_longer(SC_fill, cols = "bracharia.lacnantha":"x_5", names_to = "Species", values_to = "Sp_Pin_Hits")

SC_long$Sp_Pin_Hits <- as.numeric(SC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SC_final <- SC_long %>%
  mutate(Treatment = "C", Block = "S", Date = "20070522") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### SMW June 2007 Data Cleaning ###############################################
################################################################################
SMW <- read.csv("2007June_SMW_raw.csv", skip = 4)
...
#flip to long format
SMW_long <- pivot_longer(SMW_fill, cols = "bracharia.lacnantha":"x_4", names_to = "Species", values_to = "Sp_Pin_Hits")

SMW_long$Sp_Pin_Hits <- as.numeric(SMW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SMW_final <- SMW_long %>%
  mutate(Treatment = "MW", Block = "S", Date = "20070709") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### SMWC June 2007 Data Cleaning ##############################################
################################################################################
SMWC <- read.csv("2007June_SMWC_raw.csv", skip = 4)
...
#flip to long format
SMWC_long <- pivot_longer(SMWC_fill, cols = "bracharia.lacnantha":"x_5", names_to = "Species", values_to = "Sp_Pin_Hits")

SMWC_long$Sp_Pin_Hits <- as.numeric(SMWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SMWC_final <- SMWC_long %>%
  mutate(Treatment = "MWC", Block = "S", Date = "20070601") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### SO June 2007 Data Cleaning #################################################
################################################################################
SO <- read.csv("2007June_SO_raw.csv", skip = 4)
...
#make block, date, treatment columns
SO_final <- SO_long %>%
  mutate(Treatment = "O", Block = "S", Date = "20070710") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### SW June 2007 Data Cleaning ################################################
################################################################################
SW <- read.csv("2007June_SW_raw.csv", skip = 4)
...
#flip to long format
SW_long <- pivot_longer(SW_fill, cols = "bracharia.lacnantha":"x_4", names_to = "Species", values_to = "Sp_Pin_Hits")

SW_long$Sp_Pin_Hits <- as.numeric(SW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SW_final <- SW_long %>%
  mutate(Treatment = "W", Block = "S", Date = "20070606") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
################################################################################
#### SWC June 2007 Data Cleaning ###############################################
################################################################################
SWC <- read.csv("2007June_SWC_raw.csv", skip = 4)
...
#flip to long format
SWC_long <- pivot_longer(SWC_fill, cols = "bracharia.lacnantha":"ocimum..mint..", names_to = "Species", values_to = "Sp_Pin_Hits")

SWC_long$Sp_Pin_Hits <- as.numeric(SWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SWC_final <- SWC_long %>%
  mutate(Treatment = "WC", Block = "S", Date = "20070531") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
write.csv(final, "2007Jun_BlockS.csv", row.names = FALSE)
