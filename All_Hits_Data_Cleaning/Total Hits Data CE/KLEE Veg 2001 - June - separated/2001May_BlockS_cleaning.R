...
################################################################################
#### SC May 2001 Data Cleaning #################################################
################################################################################
SC <- read.csv("2001May_SC_raw.csv", skip = 1)
...
#flip to long format
SC_long <- pivot_longer(SC_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SC_long$Sp_Pin_Hits <- as.numeric(SC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SC_final <- SC_long %>%
  mutate(Treatment = "C", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### SMW May 2001 Data Cleaning ################################################
################################################################################
SMW <- read.csv("2001May_SMW_raw.csv", skip = 1)
...
#flip to long format
SMW_long <- pivot_longer(SMW_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SMW_long$Sp_Pin_Hits <- as.numeric(SMW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SMW_final <- SMW_long %>%
  mutate(Treatment = "MW", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
################################################################################
SMWC <- read.csv("2001May_SMWC_raw.csv", skip = 1)
...
#flip to long format
SMWC_long <- pivot_longer(SMWC_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SMWC_long$Sp_Pin_Hits <- as.numeric(SMWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SMWC_final <- SMWC_long %>%
  mutate(Treatment = "MWC", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)


################################################################################
#### SO June 2000 Data Cleaning ################################################
################################################################################
SO <- read.csv("2001May_SO_raw.csv", skip = 1)
...
#flip to long format
SO_long <- pivot_longer(SO_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SO_long$Sp_Pin_Hits <- as.numeric(SO_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SO_final <- SO_long %>%
  mutate(Treatment = "O", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
################################################################################
SW <- read.csv("2001May_SW_raw.csv", skip = 1)
...
#flip to long format
SW_long <- pivot_longer(SW_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SW_long$Sp_Pin_Hits <- as.numeric(SW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SW_final <- SW_long %>%
  mutate(Treatment = "W", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
################################################################################
SWC <- read.csv("2001May_SWC_raw.csv", skip = 1)
...
#flip to long format
SWC_long <- pivot_longer(SWC_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

SWC_long$Sp_Pin_Hits <- as.numeric(SWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
SWC_final <- SWC_long %>%
  mutate(Treatment = "WC", Block = "S", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
write.csv(final, "2001May_BlockS.csv", row.names = FALSE)
