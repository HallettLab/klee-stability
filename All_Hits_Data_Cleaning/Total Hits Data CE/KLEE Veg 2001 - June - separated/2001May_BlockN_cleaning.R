...
################################################################################
#### NC May 2001 Data Cleaning ################################################
################################################################################
NC <- read.csv("2001May_NC_raw.csv", skip = 1)
...
#flip to long format
NC_long <- pivot_longer(NC_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

NC_long$Sp_Pin_Hits <- as.numeric(NC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
NC_final <- NC_long %>%
  mutate(Treatment = "C", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### NMW May 2001 Data Cleaning ###############################################
################################################################################
NMW <- read.csv("2001May_NMW_raw.csv", skip = 1)
...
#flip to long format
NMW_long <- pivot_longer(NMW_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

NMW_long$Sp_Pin_Hits <- as.numeric(NMW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
NMW_final <- NMW_long %>%
  mutate(Treatment = "MW", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### NMWC May 2001 Data Cleaning ##############################################
################################################################################
NMWC <- read.csv("2001May_NMWC_raw.csv", skip = 1)
...
#flip to long format
NMWC_long <- pivot_longer(NMWC_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

NMWC_long$Sp_Pin_Hits <- as.numeric(NMWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
NMWC_final <- NMWC_long %>%
  mutate(Treatment = "MWC", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)


################################################################################
#### NO May 2001 Data Cleaning #################################################
################################################################################
NO <- read.csv("2001May_NO_raw.csv", skip = 1)
...
#flip to long format
NO_long <- pivot_longer(NO_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

NO_long$Sp_Pin_Hits <- as.numeric(NO_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
NO_final <- NO_long %>%
  mutate(Treatment = "O", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### NW May 2001 Data Cleaning #################################################
################################################################################
NW <- read.csv("2001May_NW_raw.csv", skip = 1)
...
#flip to long format
NW_long <- pivot_longer(NW_clean, cols = "bothriochloa":"bares", names_to = "Species", values_to = "Sp_Pin_Hits")

NW_long$Sp_Pin_Hits <- as.numeric(NW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
NW_final <- NW_long %>%
  mutate(Treatment = "W", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
################################################################################
#### NWC May 2001 Data Cleaning ################################################
################################################################################
NWC <- read.csv("2001May_NWC_raw.csv", skip = 1)
...
#make block, date, treatment columns
NWC_final <- NWC_long %>%
  mutate(Treatment = "WC", Block = "N", Date = "20010501") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
write.csv(final, "2001May_BlockN.csv", row.names = FALSE)
