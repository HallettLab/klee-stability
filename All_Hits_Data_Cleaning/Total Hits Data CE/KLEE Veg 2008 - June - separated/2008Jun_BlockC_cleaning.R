...
################################################################################
#### CC June 2008 Data Cleaning ################################################
################################################################################
CC <- read.csv("2008Jun_CC_raw.csv", skip = 4)
...
#flip to long format
CC_long <- pivot_longer(CC_fill, cols = "bracharia.lacnantha":"x4", names_to = "Species", values_to = "Sp_Pin_Hits")

CC_long$Sp_Pin_Hits <- as.numeric(CC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
CC_final <- CC_long %>%
  mutate(Treatment = "C", Block = "C", Date = "20080611") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### CMW June 2008 Data Cleaning ###############################################
################################################################################
CMW <- read.csv("2008Jun_CMW_raw.csv", skip = 4)
...
#flip to long format
CMW_long <- pivot_longer(CMW_fill, cols = "bracharia.lacnantha":"x4", names_to = "Species", values_to = "Sp_Pin_Hits")

CMW_long$Sp_Pin_Hits <- as.numeric(CMW_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
CMW_final <- CMW_long %>%
  mutate(Treatment = "MW", Block = "C", Date = "20080612") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### CMWC June 2008 Data Cleaning ##############################################
################################################################################
CMWC <- read.csv("2008Jun_CMWC_raw.csv", skip = 4)
...
#remove first row as it now only contains duplicate info. remove blank 2nd col
CMWC_t <- t_CMWC[-1,-2]
...
#flip to long format
CMWC_long <- pivot_longer(CMWC_fill, cols = "bracharia.lacnantha":"x4", names_to = "Species", values_to = "Sp_Pin_Hits")

CMWC_long$Sp_Pin_Hits <- as.numeric(CMWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
CMWC_final <- CMWC_long %>%
  mutate(Treatment = "MWC", Block = "C", Date = "20080610") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### CO June 2008 Data Cleaning ################################################
################################################################################
CO <- read.csv("2008Jun_CO_raw.csv", skip = 4)
...
#make block, date, treatment columns
CO_final <- CO_long %>%
  mutate(Treatment = "O", Block = "C", Date = "20080611") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)




################################################################################
#### CW June 2008 Data Cleaning ################################################
################################################################################
CW <- read.csv("2008Jun_CW_raw.csv", skip = 4)
...
#make block, date, treatment columns
CW_final <- CW_long %>%
  mutate(Treatment = "W", Block = "C", Date = "20080610") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)



################################################################################
#### CWC June 2008 Data Cleaning ###############################################
################################################################################
CWC <- read.csv("2008Jun_CWC_raw.csv", skip = 4)
...
#flip to long format
CWC_long <- pivot_longer(CWC_fill, cols = "bracharia.lacnantha":"x_4", names_to = "Species", values_to = "Sp_Pin_Hits")

CWC_long$Sp_Pin_Hits <- as.numeric(CWC_long$Sp_Pin_Hits) #make the # of pin hits numeric

#make block, date, treatment columns
CWC_final <- CWC_long %>%
  mutate(Treatment = "WC", Block = "C", Date = "20080609") #add columns for treatment, block, date (using the first of Feb as sample day is not specified)
...
write.csv(final, "2008Jun_BlockC.csv", row.names = FALSE)
