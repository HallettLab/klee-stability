...
stability_mechanisms$wildlife[stability_mechanisms$TREATMENT %in% c('W','WC')] <- 1
...
fitsth_bf <- lmer(stability~cows+mega + (1|BLOCK), data = stability_mechanisms, na.action = "na.fail")
summary(fitsth_bf)
...
#fitgrst_b2 <- aov(stability~cows*wildlife + BLOCK, data = stability_mechanisms)
#summary(fitgrst_b2)
#fitgrst_b2
...
fitrih_best <- lmer(meanrich~1 + (1|BLOCK), data = stability_mechanisms, na.action = "na.fail")
summary(fitrih_best)
```
...
#cor.test(stability_mechanisms$meanpopstab, stability_mechanisms$mean_dom)
## 0.02 

cor.test(stability_mechanisms$b5classicVR, stability_mechanisms$classicVR)
## 0.963 (significant)
...
fitstabcvrpopst <- lmer(stability~classicVR + meanpopstab + (1|BLOCK), data = stability_mechanisms, na.action = "na.fail")

summary(fitstabcvrpopst)
