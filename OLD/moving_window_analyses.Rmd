...
sm <- read.csv("stability_mechanisms.csv")
msm <- read.csv("mean_stability_mech.csv")
vrplot <- read.csv("VR_plotting.csv")
...
msm$TREATMENT <- as.factor(msm$TREATMENT)
vrplot$TREATMENT <- as.factor(vrplot$TREATMENT)
...
stab_mw_tx <- left_join(mwstability, treats, by="Unique_ID") #%>%
  #group_by(TREATMENT, timestep, window_size, BLOCK, Unique_ID) %>%
...
## make one data frame to export and use for SEM tests
...
mw10all <- left_join(jk3, dscore10, by = "timestep")


write.csv(mw10all, file = "mw_forSEM.csv")
```
...
stab_pSEM_list = list(
  ...
  st_tb = lmer(stability~Dscore+TREATMENT+ classicVR + mean_popst + 
              richness + (1|BLOCK) + (1|Unique_ID), data=stabmechmw10, 
              na.action = "na.fail"),
  
  ## predicting variance ratio
  cvr_sem = lmer(classicVR~Dscore+TREATMENT + richness + (1|BLOCK) + 
                     (1|Unique_ID), data = stabmechmw10, na.action = "na.fail"),
  
  ## predicting population stability
  ps_sem = lmer(mean_popst~Dscore+TREATMENT + (1|BLOCK) + (1|Unique_ID), 
                  data = drpopst10yr, na.action = "na.fail"),
  
  ## predicting species richness
  ri_sem = lmer(richness~Dscore+TREATMENT + (1|BLOCK) + (1|Unique_ID), 
                  data = drrich10yr, na.action = "na.fail")
...
sem.fit(stab_pSEM_list, st_tb)

# Evaluate path significance using unstandardized coefficients
sem.coefs(stab_pSEM_list, st_tb, standardize = "none")

# Obtain standardized regression coefficients
sem.coefs(stab_pSEM_list, st_tb, standardize = "scale")

# Explore individual model fits
sem.model.fits(stab_pSEM_list)
```
...
## Figure 2: Stability (full ts, 5yr & 10yr)
...
## Figure 3: One Dscore-stability - cvr regression figure
...
## Figure S5: Stability-VR Correlations
#```{r, fig.width=7, fig.height=4}
w10 <- ggplot(figdrst, aes(x=cVR, y=stability)) +
  geom_point() +
  ...
  coord_cartesian(xlim = c(0.5,4.5), ylim = c(2.5,12.6)) +
  geom_smooth(method = "lm", fullrange = T, se = FALSE, mapping = aes(color = TREATMENT), alpha = 0.75) +
  scale_color_scico_d(palette = "batlow", direction = -1) +
  geom_point(aes(fill=TREATMENT), 
             colour="black",pch=21, size=2.5) +
  scale_fill_scico_d(palette = "batlow", direction = -1) +
...
w5 <- ggplot(figdrst5, aes(x=cVR, y=stability)) +
  geom_point() +
